...
if (import.meta.vitest) {
  ...
  it('should find hint for a 1s either side a 0', () => {
    ...
    const actual = hintLineTriplet(values)
 ...
 */
const hintLineNoForcingThreeInARow = (
  values: TangoValue[]
): LineCellValue<TangoValue.NonEmpty> | null => {
...
if (import.meta.vitest) {
  ...
  it('hints when a symbol encloses 4 cells with an empty inner left', () => {
    ...
    const actual = hintLineNoForcingThreeInARow(values)
...
export const getConsecutiveCellsHint = (
  ...
  for (const { cells, line } of Grid.cellsByLine(grid)) {
    const tripletMove = hintLineTriplet(cells)
    ...
    const noForcingThreeInARowMove = hintLineNoForcingThreeInARow(cells)
